...
The x-axis displays the first principal component (PC1) and the y-axis the second principal component (PC2). The observations are shown in the plot in green with the 8 variables shown in black. As we can see, the plot above is hard to read. It seems that GNI has the strongest effect in PC1. Qatar is clearly different from other countries, but otherwise the plot is difficult to interpret. Therefore, it is a good idea to try the PCA again with scaled data. 

...
mehu <- ggplot(data = alc, aes(x = Medu_cat, fill = high_use)) + 
  ...
  xlab("Mothers education from no education (1) to highest education (4)") +
...
fehu <- ggplot(data = alc, aes(x = Fedu_cat, fill = high_use)) + 
  ...
  xlab("Fathers education from no education (1) to highest education (4)") +
  scale_fill_brewer(palette="Dark2") +
  labs(fill="High Alcohol Use") 
fehu
...
```
The Coefficients table shows that only quality of family relations (**famrel**) is statistically significant (p<0.05). The coefficient estimate of the variable **famrel** is negative (b=-0.2817) meaning that on step increase in family relation quality decreases the probability of being high-user of alcohol to 0.72. The odds ratio (OR) for famrel is 0.754. The living area (**address**) correlates with the high use of alcohol, but it is not statistically significant (p=0.0695).

<br>

It's a bit boring to have only one variable explaining the high alcohol use in this model, but below I'm exploring the predictive power of the model. In my opinion, this model does not give much more information compared to the simple guessing from the previous bar plots and frequency tables. 
